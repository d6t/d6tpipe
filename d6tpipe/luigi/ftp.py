...
class RemoteFileSystem(luigi.target.FileSystem):
    ...
    def close_del(self):
        ...
        try:
            if self.sftp:
                ...
                self._ftp_close()
            print('closed ftp cnxn')
        except:
            warnings.warn('unable to close ftp connection')
    ...
    def get(self, path, local_path):
        ...
        try:
            ...
            warnings.warn('rename failed, {} - {}'.format(tmp_local_path,local_path))
        if pathlib.Path(tmp_local_path).exists():
            warnings.warn('temp file still exists {}'.format(tmp_local_path))
    ...
    def listdir_attr(self, path='.'):
        ...
        if self.sftp:
            contents = self._sftp_listdir_attr(path)
        else:
            raise NotImplementedError()
            contents = self._ftp_listdir_attr(path)
